...
# Ivan's Change: Add farmlive_training_task
# from celery_tasks.tasks import get_all_universities_task, get_university_task
from celery_tasks.tasks import get_all_universities_task, get_university_task, farmlive_training_task
from config.celery_utils import get_task_info

# Ivan's change: IMport Job from schema
from schemas.schemas import Country, Job
...
# Ivan's Change: Add training_async endpoint for farmlive_predict_task
@router.post("/training_async")
async def farmlive_training_async(job: Job):
    """
    Run famrlive training job in Async way with 1 job input. 
    It just returns the task id, which can later be used to get the result.
    """
    task = farmlive_training_task.apply_async(args=[job.jobs])
    return JSONResponse({"task_id": task.id})

# Ivan's Change: Add training sync endpoint for farmlive_predict_task
@router.post("/training")
async def farmlive_training_sync(job: Job):
    """
    Run famrlive training job in Sync way with 1 job input. 
    It just returns the status "Done".
    """
    universities.run_training(job)
    results="Done"
    return results
