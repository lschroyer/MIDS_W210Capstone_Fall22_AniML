...
def form_get(request: Request):
    ...
    return templates.TemplateResponse('review.html', context={'request': request, 'model_predictions': show_model_table_initial})
...
@router.get("/review_random_file", response_class=HTMLResponse)
def random_files(request: Request):

    random_file = []
    for i in range(5):
        random_file.append(random_script.list_random_files())

    return templates.TemplateResponse('review.html', 
        context={'request': request,
            'random_files_names': random_file
        })
